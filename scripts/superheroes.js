...
/**
 * Редактор списка. Один редактор используется для способностей, особенностей и снаряжения.
 * Внутри окна можно добавлять любое количество одинаковых карточек: название + описание.
 */
function showListEditor(actor,type){
  const labels={powers:"Способности",traits:"Особенности",gear:"Снаряжение"};
  const current=clone(mergeDefaults(actor.system)[type]||[]);
  const title=labels[type]||"Список";
  const rowHTML=(item={},index=0)=>`<div class="sh-list-editor-row" data-editor-index="${index}" data-editor-original-index="${index}">
    <div class="sh-list-editor-row-head"><span>ЭЛЕМЕНТ ${index+1}</span><button type="button" class="sh-list-editor-remove" data-editor-remove title="Удалить">×</button></div>
    <label>Название</label>
    <input class="sh-list-editor-name" type="text" value="${safe(item.name)}" placeholder="Например: Рация">
    <label>Описание</label>
    <textarea class="sh-list-editor-description" placeholder="Введите описание">${safe(item.description)}</textarea>
  </div>`;
  const initial=current.map((item,i)=>rowHTML(item,i)).join("");
  const content=`<div class="sh-list-editor" data-list-type="${type}">
    <div class="sh-list-editor-toolbar">
      <div><b>${title}</b><small>Добавляйте элементы кнопкой «＋». У каждого элемента есть название и описание.</small></div>
      <button type="button" class="sh-list-editor-add" data-editor-add title="Добавить элемент">＋</button>
    </div>
    <div class="sh-list-editor-rows" data-editor-rows>${initial}</div>
    <div class="sh-list-editor-empty" data-editor-empty ${current.length?"hidden":""}>Нажмите «＋», чтобы создать первый элемент.</div>
  </div>`;

  let dialog;
  const addRow=(root,item={})=>{
    const rows=root.querySelector("[data-editor-rows]");
    const index=rows.querySelectorAll(".sh-list-editor-row").length;
    rows.insertAdjacentHTML("beforeend",rowHTML(item,index));
    root.querySelector("[data-editor-empty]")?.setAttribute("hidden","");
    const last=rows.lastElementChild;
    last?.querySelector(".sh-list-editor-name")?.focus();
  };
  const renumber=rows=>rows.querySelectorAll(".sh-list-editor-row").forEach((row,i)=>{
    row.dataset.editorIndex=i;
    const label=row.querySelector(".sh-list-editor-row-head span");
    if(label)label.textContent=`ЭЛЕМЕНТ ${i+1}`;
  });

  dialog=new Dialog({
    title:`${title} — редактор`,
    content,
    buttons:{
      save:{
        label:"Сохранить",
        callback:async html=>{
          const result=[];
          html.find(".sh-list-editor-row").each((_,row)=>{
            const name=String(row.querySelector(".sh-list-editor-name")?.value||"").trim();
            const description=String(row.querySelector(".sh-list-editor-description")?.value||"").trim();
            if(!name)return;
            const originalIndex=Number(row.dataset.editorOriginalIndex);
            const original=(type==="powers" && Number.isInteger(originalIndex) && originalIndex>=0) ? (current[originalIndex]||{}) : {};
            result.push({...original,name,description});
          });
          await actor.update({[`system.${type}`]:result});
        }
      },
      cancel:{label:"Отмена"}
    },
    ...
    render:html=>{
      ...
      root.addEventListener("click",event=>{
        ...
        if(add){event.preventDefault();addRow(root);return;}
        const remove=event.target.closest("[data-editor-remove]");
        if(remove){
          event.preventDefault();
          remove.closest(".sh-list-editor-row")?.remove();
          const rows=root.querySelector("[data-editor-rows]");
          renumber(rows);
          if(!rows.querySelector(".sh-list-editor-row"))root.querySelector("[data-editor-empty]")?.removeAttribute("hidden");
        }
      });
    }
  });
...
class SuperheroesActorSheet extends ActorSheet {
  ...
  getData(options={}){
    ...
    data.system=system;
    data.derived={maxHP,maxFocus,maxKarma:system.rank,initiative:statMod(system.stats.vigilance.value)+system.initiative.bonus,stats,speed:speedDerived(this.actor)};
  ...
  activateListeners(html){
    ...
    html.on("click.superheroes","[data-action]",async e=>{
      ...
      try{
        ...
        if(a==="add-power")return showListEditor(this.actor,"powers");
        if(a==="add-trait")return showListEditor(this.actor,"traits");
        if(a==="add-gear")return showListEditor(this.actor,"gear");
        if(a==="edit-power")return showListEditor(this.actor,"powers");
        if(a==="edit-trait")return showListEditor(this.actor,"traits");
        if(a==="edit-gear")return showListEditor(this.actor,"gear");
