...
// === ЖЕЛЕЗОБЕТОННАЯ ЛОГИКА ДОБАВЛЕНИЯ ИНИЦИАТИВЫ В БОЙ ===
async function make3d6Roll(actor, formula, flavor, flags={}){
  ...
  await roll.evaluate(); // V12: без async: true
  ...
  if (flags.kind === "initiative") {
    // 1. Убеждаемся, что трекер боя существует. Если нет — создаем его!
    let combat = game.combat;
    if (!combat) {
      combat = await Combat.create({ scene: canvas.scene?.id, active: true });
    }

    // 2. Ищем, есть ли актер уже в этом трекере
    let combatant = combat.combatants.find(c => c.actorId === actor.id);
    
    if (!combatant) {
      // 3. Если актера нет, находим его токен на карте и вписываем его в бой
      ...
      // 4. Если актер уже есть в трекере, просто обновляем ему цифру
...
async function rerollSuperheroesDie(messageId,dieIndex,mode){
  ...
  await newRoll.evaluate(); // V12: без async: true
  ...
  // ПЕРЕБРОС ИНИЦИАТИВЫ: обновляем трекер, если бой существует
...
async function rollDamageFromMessage(messageId){
  ...
  await damageRoll.evaluate(); // V12: без async: true
...
Hooks.once("init",()=>{
  ...
  // ПЕРЕОПРЕДЕЛЕНИЕ БРОСКА ИНИЦИАТИВЫ В БОЕВОМ ТРЕКЕРЕ
...
// ==========================================
// ИНСТРУМЕНТ: БЫСТРОЕ ДОБАВЛЕНИЕ В БОЙ (БЕЗ НАСТРОЕК, НАПРЯМУЮ К ИНТЕРФЕЙСУ)
// ==========================================
...
Hooks.on("getSceneControlButtons", (controls) => {
  ...
  if (tokenControls) {
    tokenControls.tools.push({
      ...
      toggle: true,
      active: false,
      onClick: () => {} // Фаундри сама запоминает, нажата кнопка или нет
    });
  }
});
...
Hooks.on("controlToken", async (token, controlled) => {
  if (!game.user.isGM) return;
  // Срабатывает только когда мы ВЫДЕЛЯЕМ токен, а не снимаем выделение
  if (!controlled) return;

  // Ищем нашу кнопку в интерфейсе и смотрим её статус напрямую
  const tokenControls = ui.controls.controls.find(c => c.name === "token");
  const tool = tokenControls?.tools.find(t => t.name === "fast-combat");
  
  if (tool && tool.active) {
    // В V12 эта команда сама создаст бой, если его нет, и добавит токен
