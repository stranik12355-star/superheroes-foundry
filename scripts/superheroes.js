...
async function rerollSuperheroesDie(messageId,dieIndex,mode){
  ...
  const formulaReg=/(\d)d(\d|s).*/;
  const groups=formulaReg.exec(targetRoll._formula)?.groups;
  if(!groups) throw new Error("Не удалось определить выбранную кость");
  targetDie.number=2;
  const targetFormula=`${targetDie.number}d${groups.dieType}`;
...
class SuperheroesActorSheet extends ActorSheet {
  ...
  async _editListItem(type,index){const s=mergeDefaults(this.actor.system),item=index>=0?(s[type][index]||{}):{name:"",description:""},power=type==="powers";const content=power?`<div class="sh-dialog"><label>Название</label><input id="name" value="${safe(item.name)}"><label>Тип перемещения</label><input id="movement" value="${safe(item.movement)}"><label>Стоимость</label><input id="cost" value="${safe(item.cost)}"><label>Дальность</label><input id="range" value="${safe(item.range)}"><label>Тип урона</label><input id="damageType" value="${safe(item.damageType)}"><label>Описание</label><textarea id="description">${safe(item.description)}</textarea></div>`:`<div class="sh-dialog"><label>Название</label><input id="name" value="${safe(item.name)}"><label>Описание</label><textarea id="description">${safe(item.description)}</textarea></div>`;showDialog(content,`${index<0?"Добавить":"Изменить"} — ${power?"Способность":type==="traits"?"Черта":"Снаряжение"}`,async h=>{const next={name:h.find("#name").val(),description:h.find("#description").val()};if(power){next.movement=h.find("#movement").val();next.cost=h.find("#cost").val();next.range=h.find("#range").val();next.damageType=h.find("#damageType").val();}const arr=clone(s[type]);if(index<0)arr.push(next);else arr[index]=next;await this.actor.update({[`system.${type}`]:arr});});}
