...
function mergeDefaults(source={}) {
  ...
  for(const p of ["health","focus"]){
    ...
    const max=resourceMax(p==="health"?d.stats.endurance.value:d.stats.vigilance.value,d[p].bonus);
    d[p].value=Math.min(d[p].value,max);
  }
...
async function createInitiativeRoll(actor){
  const system=mergeDefaults(actor.system);
  const modifier=statMod(system.stats.vigilance.value)+Number(system.initiative.bonus||0);
  const roll=new Roll("1d6 + 1ds + 1d6");
  await roll.evaluate({async:true});
  const values=getThreeDice(roll);
  const total=values.reduce((a,v)=>a+v.effective,0)+modifier;
  const label="Инициатива";
  const content=diceCard(label,values,total,values[1]?.critical,"");
  const message=await ChatMessage.create({
    speaker:ChatMessage.getSpeaker({actor}),
    flavor:label,
    content,
    rolls:[roll],
    flags:{superheroes:{type:"3d6",kind:"initiative",modifier,dice:values.map(v=>v.result)}}
  });
  await message.update({content:diceCard(label,values,total,values[1]?.critical,message.id)});
  return roll;
}
