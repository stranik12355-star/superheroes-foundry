...
const SH_DEFAULTS = {
  ...
  powers: [], traits: [], gear: [],
  tokenGallery: ["", "", "", "", ""] // 5 слотов для обликов
};
...
class SuperheroesActorSheet extends ActorSheet {
  ...
  getData(options={}){
    ...
    // Передаем текущий токен в шаблон для подсветки активного облика
  ...
  /* ТЕПЕРЬ ПОРТРЕТ МЕНЯЕТ ТОЛЬКО АРТ В ЧАРНИКЕ (ТОКЕН НЕ ТРОГАЕТ) */
  _onEditImage(event) {
  ...
  activateListeners(html){
    ...
    /* === ЛЕВАЯ КНОПКА ПО ОБЛИКУ (СМЕНА ТОКЕНА) === */
    html.on("click", ".token-slot", async e => {
      ...
      if (!imgPath) {
        // Если пусто - предлагаем выбрать файл
        new FilePicker({
          type: "image",
          callback: async path => {
        ...
        // Если есть картинка - превращаемся!
    ...
    /* === ПРАВАЯ КНОПКА ПО ОБЛИКУ (ИЗМЕНИТЬ АРТ В СЛОТЕ) === */
...
Hooks.on("preCreateActor", function(actor) {
  ...
  actor.updateSource({
    ...
    prototypeToken: {
      actorLink: true,                            
      displayName: 20, // 20 = При наведении Владельцем (OWNER_HOVER)
      displayBars: 20, // 20 = При наведении Владельцем (OWNER_HOVER)
