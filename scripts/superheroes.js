...
async function make3d6Roll(actor, formula, flavor, flags={}){
  ...
  if (flags.kind === "initiative" && game.combat) {
    ...
    // Если актёра нет в трекере, жёстко добавляем его
    if (!combatant) {
    ...
    // Если актёр уже в трекере, обновляем его цифру
    else {
...
async function rerollSuperheroesDie(messageId,dieIndex,mode){
  ...
  // ПЕРЕБРОС ИНИЦИАТИВЫ: обновляем трекер при преимуществе/помехе
...
Hooks.once("init",()=>{
  ...
  // РЕГИСТРИРУЕМ НАСТРОЙКУ ТУТ (В ИНИТЕ!), ЧТОБЫ МЕНЮ БОЛЬШЕ НИКОГДА НЕ ЛОМАЛОСЬ
  game.settings.register("core", "shFastCombat", {
    name: "Быстрое добавление в бой",
    scope: "client",
    config: false,
    type: Boolean,
    default: false
...
Hooks.on("getSceneControlButtons", (controls) => {
  ...
  if (tokenControls) {
    tokenControls.tools.push({
      ...
      icon: "fas fa-bolt", 
      toggle: true,
      active: game.settings.get("core", "shFastCombat"),
      onClick: (toggled) => {
        game.settings.set("core", "shFastCombat", toggled);
      }
    });
  }
});
...
Hooks.on("controlToken", async (token, controlled) => {
  // Выполняется только для ГМа, только если токен выделили, и если включен режим
  if (controlled && game.user.isGM && game.combat && game.settings.get("core", "shFastCombat")) {
