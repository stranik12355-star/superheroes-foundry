...
class SuperheroesActorSheet extends ActorSheet {
  ...
  activateListeners(html) {
    ...
    // ===== НОВОЕ: Отправка макроса броска на панель =====
...
// ===== НОВОЕ: Хук для перехвата сброса на панель макросов =====
Hooks.on("hotbarDrop", (bar, data, slot) => {
  if (data.type === "Macro" && data.system === "superheroes") {
      createSuperheroesMacro(data, slot);
      return false; // Запрещаем стандартное действие Foundry
  }
});
...
async function createSuperheroesMacro(data, slot) {
  ...
  // Эмодзи в названии (будет видно на макросе)
  const name = isAttack ? `⚔️ ${statName}` : `🎲 ${statName}`;
  
  // Берем иконку напрямую из твоей папки assets (чтобы она точно совпала с листом персонажа)
  const img = `systems/superheroes/assets/icons/${data.stat}.png`;

  // Команда, которая будет выполняться при клике на макрос
  const command = `SuperheroesSystem.executeMacro("${data.actorId}", "${data.stat}", "${data.macroType}");`;

  // Ищем, есть ли уже такой макрос
  ...
  // Назначаем созданный/найденный макрос на слот
