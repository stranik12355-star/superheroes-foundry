...
Hooks.once("init",()=>{
  ...
  // --- БЕЗОПАСНАЯ ИНТЕГРАЦИЯ ИНИЦИАТИВЫ В БОЙ ---
  // 1. Говорим трекеру, по какой формуле делать бросок:
...
// 2. Перехватываем сообщение в чат ПЕРЕД созданием, чтобы оформить его в нашем стиле
Hooks.on("preCreateChatMessage", (message, data, options, userId) => {
  const roll = message.rolls?.[0];
  // Проверяем, что это инициатива из трекера боя и это наш кастомный 3d6
  if (roll && isSuperheroesRoll(roll) && message.flavor === "Инициатива") {
      message.updateSource({
          "flags.superheroes": { kind: "initiative", threeD6: true }
      });
  }
});
