...
// === ИНИЦИАТИВА: Добавление в трекер ===
async function make3d6Roll(actor, formula, flavor, flags={}){
  ...
  if (flags.kind === "initiative") {
    ...
    const finalInit = roll._total === 1 ? 6 : roll._total; // Безопасное получение итога
...
async function rerollSuperheroesDie(messageId,dieIndex,mode){
  ...
  // ПЕРЕБРОС ИНИЦИАТИВЫ: Синхронизация с трекером
  const flags = chatMessage.flags?.superheroes || {};
  if (flags.kind === "initiative" && game.combat) {
    const speaker = chatMessage.speaker;
    let combatant = null;
    
    // Ищем точечно по токену или актеру
    if (speaker.token) combatant = game.combat.combatants.find(c => c.tokenId === speaker.token);
    if (!combatant && speaker.actor) combatant = game.combat.combatants.find(c => c.actorId === speaker.actor);
    
    if (combatant) {
      // Принудительно вычисляем финал, игнорируя кэширование Фаундри
      const finalTotal = roll._total === 1 ? 6 : roll._total;
      await game.combat.updateEmbeddedDocuments("Combatant", [{ _id: combatant.id, initiative: finalTotal }]);
    }
  }
...
// ==========================================
// ИНСТРУМЕНТ: БЫСТРОЕ ДОБАВЛЕНИЕ В БОЙ (БЕЗ ОШИБОК И БАЗ ДАННЫХ)
// ==========================================

// 1. Создаем локальную переменную в памяти. Это решает все проблемы синхронизации!
...
Hooks.on("getSceneControlButtons", (controls) => {
  ...
  if (tokenControls) {
    tokenControls.tools.push({
      ...
      onClick: (toggled) => {
        // Просто меняем переменную в памяти, когда нажимаем кнопку меню
...
Hooks.on("controlToken", async (token, controlled) => {
  // Если выделяем токен (не снимаем выделение), мы ГМ, и кнопка активна — бросаем в бой
