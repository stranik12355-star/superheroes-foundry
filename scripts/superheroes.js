...
class SuperheroesActorSheet extends ActorSheet {
  ...
  async _editListItem(type,index){
    if(!["powers","traits","gear"].includes(type)) return;
    const s=mergeDefaults(this.actor.system), existing=index>=0?(s[type]?.[index]||{}):{name:"",description:""};
    const label=type==="powers"?"Способность":type==="traits"?"Черта":"Снаряжение", power=type==="powers";
    const esc=safe;
    const content=`<div class="sh-dialog list-item-dialog"><label>Название</label><input id="item-name" type="text" value="${esc(existing.name||"")}" placeholder="Название">${power?`<label>Тип перемещения</label><input id="item-movement" value="${esc(existing.movement||"")}"><label>Стоимость</label><input id="item-cost" value="${esc(existing.cost||"")}"><label>Дальность</label><input id="item-range" value="${esc(existing.range||"")}"><label>Тип урона</label><input id="item-damage" value="${esc(existing.damageType||"")}">`:""}<label>Описание</label><textarea id="item-description" rows="7" placeholder="Описание">${esc(existing.description||"")}</textarea></div>`;
    await showDialog(content,`${index<0?"Добавить":"Изменить"} — ${label}`,async h=>{
      const name=String(h.find("#item-name").val()??"").trim(); if(!name){ui.notifications.warn("Введите название.");return;}
      const arr=clone(s[type]||[]), next={...(index>=0?(arr[index]||{}):{}),name,description:String(h.find("#item-description").val()??"").trim()};
      if(power){next.movement=String(h.find("#item-movement").val()??"").trim();next.cost=String(h.find("#item-cost").val()??"").trim();next.range=String(h.find("#item-range").val()??"").trim();next.damageType=String(h.find("#item-damage").val()??"").trim();}
      if(index>=0&&index<arr.length)arr[index]=next;else arr.push(next); await this.actor.update({[`system.${type}`]:arr});
    });
  }
  ...
  _inlineEditorHTML(type,item={},index=-1){
    const name=safe(item.name||"");
    const description=safe(item.description||"");
    return `<div class="inline-list-editor" data-inline-editor data-type="${type}" data-index="${index}">
      <div class="inline-list-editor-title"><span>${this._listLabel(type).toUpperCase()}</span><span class="inline-list-editor-actions">
        <button type="button" data-action="save-list-item" title="Сохранить">✓</button>
        <button type="button" data-action="cancel-list-item" title="Отмена">×</button>
      </span></div>
      <label>НАЗВАНИЕ</label>
      <input class="inline-list-name" type="text" value="${name}" placeholder="Название">
      <label>ОПИСАНИЕ</label>
      <textarea class="inline-list-description" placeholder="Описание">${description}</textarea>
    </div>`;
  }
  async _addInlineListItem(type){
    if(!["powers","traits","gear"].includes(type))return;
    const root=this.element?.[0]||this.element;
    const panel=root?.querySelector(`.list-panel[data-list-type="${type}"]`);
    const container=panel?.querySelector(`[data-list-items="${type}"]`);
    if(!container)return;
    const existing=container.querySelector("[data-inline-editor]");
    if(existing){existing.querySelector(".inline-list-name")?.focus();return;}
    panel.querySelector("[data-list-empty]")?.remove();
    container.insertAdjacentHTML("beforeend",this._inlineEditorHTML(type,{},-1));
    const editor=container.lastElementChild;
    editor?.querySelector(".inline-list-name")?.focus();
  }
  async _editInlineListItem(type,index){
    if(!["powers","traits","gear"].includes(type)||!Number.isInteger(index))return;
    const root=this.element?.[0]||this.element;
    const container=root?.querySelector(`[data-list-items="${type}"]`);
    if(!container)return;
    const existing=container.querySelector("[data-inline-editor]");
    if(existing){existing.querySelector(".inline-list-name")?.focus();return;}
    const item=mergeDefaults(this.actor.system)[type]?.[index];
    const row=container.querySelector(`[data-list-item][data-index="${index}"]`);
    if(!item||!row)return;
    row.insertAdjacentHTML("beforebegin",this._inlineEditorHTML(type,item,index));
    row.remove();
    container.querySelector(`[data-inline-editor][data-index="${index}"] .inline-list-name`)?.focus();
  }
  async _saveInlineListItem(editor){
    if(!editor)return;
    const type=editor.dataset.type,index=Number(editor.dataset.index);
    const name=String(editor.querySelector(".inline-list-name")?.value||"").trim();
    const description=String(editor.querySelector(".inline-list-description")?.value||"").trim();
    if(!name){ui.notifications.warn(`Введите название ${this._listLabel(type)}.`);editor.querySelector(".inline-list-name")?.focus();return;}
    const s=mergeDefaults(this.actor.system),arr=clone(s[type]||[]);
    if(index>=0 && index<arr.length){arr[index]={...arr[index],name,description};}
    else arr.push({name,description});
    await this.actor.update({[`system.${type}`]:arr});
  }
  _cancelInlineListItem(editor){
    if(!editor)return;
    const type=editor.dataset.type,index=Number(editor.dataset.index);
    if(index>=0){this.render(false);return;}
    const container=editor.parentElement;editor.remove();
    if(!container?.querySelector("[data-list-item]")){
      container?.insertAdjacentHTML("beforeend",`<div class="empty" data-list-empty>Нет ${type==="powers"?"способностей":type==="traits"?"особенностей":"снаряжения"}. Нажмите ＋, чтобы добавить.</div>`);
    }
  }
  ...
  row.querySelectorAll(".sh-list-editor-name,.sh-list-editor-description").forEach(i=>i.readOnly=!!locked);
