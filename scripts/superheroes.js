...
Hooks.once("init",()=>{
  ...
  // --- ЖЕСТКАЯ ПЕРЕЗАПИСЬ ИНИЦИАТИВЫ ФАУНДРИ ---
  // Чтобы трекер всегда бросал красивую карточку 3d6
  const _originalRollInitiative = Combat.prototype.rollInitiative;
  Combat.prototype.rollInitiative = async function(ids, options={}) {
    ...
    for (let c of combatants) {
      if (!c.actor) continue;
      const s = mergeDefaults(c.actor.system);
      const modifier = statMod(s.stats.vigilance.value) + Number(s.initiative.bonus || 0);
      
      const roll = new SuperheroesRoll(`{1d6,1ds,1d6} + ${modifier}`, c.actor.getRollData());
      roll.options.flavor = "Инициатива";
      await roll.evaluate({async: true});
      
      // Создаем нашу красивую 3d6 карточку в чате
      await roll.toMessage({
        speaker: ChatMessage.getSpeaker({actor: c.actor, token: c.token}),
        flags: {superheroes: {kind: "initiative", modifier: modifier, threeD6: true}}
      });
...
Hooks.on("getSceneControlButtons", (controls) => {
  ...
  if (tokenControls) {
    tokenControls.tools.push({
      name: "combat-add",
      title: "Добавить/Удалить из боя (Клик по токену)",
      icon: "fas fa-swords",
      toggle: true,
      active: false,
...
Hooks.on("hoverToken", (token, hovered) => {
  ...
  const updatePos = () => {
      ...
      const pos = canvas.clientCoordinatesFromCanvas({
        x: token.center.x, 
        y: token.y
      });
