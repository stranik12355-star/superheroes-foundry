...
/* ===== БАЗА ДАННЫХ — ОКНО И ЛОГИКА ===== */

class SuperheroesDatabaseApp extends Application {
    constructor(options = {}) {
        super(options);
    ...
    static get defaultOptions() {
        return foundry.utils.mergeObject(super.defaultOptions, {
            id: "sh-database",
            title: "БАЗА ДАННЫХ",
            template: "systems/superheroes/templates/database-window.hbs",
            width: 500,
            height: 400,
            resizable: true,
            dragDrop: [{ dragSelector: ".db-drag-source", dropSelector: null }],
            classes: ["superheroes", "sh-database-window"]
        });
    }
    ...
    activateListeners(html) {
        ...
        // Закрытие окна
        html.find(".window-close").on("click", () => {
            this.close();
        });

        // Сворачивание по двойному клику на заголовок
        html.find(".window-header").on("dblclick", () => {
            this.minimized = !this.minimized;
            this.element.toggleClass("minimized", this.minimized);
            this.setPosition({ height: this.minimized ? 40 : 400 });
        });

        // Drag & Drop для макросов
        html.find(".db-drag-source").on("dragstart", (e) => {
            const dragData = {
                type: "Macro",
                system: "superheroes",
                macroType: "database",
    ...
    _getHeaderButtons() {
        return []; // Убираем стандартные кнопки Foundry
    }
}

// Хук для открытия Базы Данных из листа персонажа
Hooks.on("renderSuperheroesActorSheet", (app, html, data) => {
    html.on("click", "[data-action='open-database']", async (e) => {
        e.preventDefault();
        const databaseApp = new SuperheroesDatabaseApp({ actorId: app.actor.id });
        databaseApp.render(true);
    });
});

// Объединённый хук для перетаскивания в хотбар (характеристики + база данных)
Hooks.on("hotbarDrop", (bar, data, slot) => {
    if (data.type === "Macro" && data.system === "superheroes") {
        if (data.macroType === "database") {
            createDatabaseMacro(data, slot);
            return false;
        }
        if (data.macroType === "attack" || data.macroType === "noncombat") {
            createSuperheroesMacro(data, slot);
            return false;
        }
    }
...
async function createDatabaseMacro(data, slot) {
    const actor = game.actors.get(data.actorId);
    if (!actor) return;

    const name = `💾 База Данных — ${actor.name}`;
    // Если иконка не найдётся, используем стандартную
    const img = "systems/superheroes/assets/icons/database.png";

    const command = `new SuperheroesDatabaseApp({ actorId: "${data.actorId}" }).render(true);`;
