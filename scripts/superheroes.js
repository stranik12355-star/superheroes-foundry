...
class SuperheroesActorSheet extends ActorSheet {
  ...
  activateListeners(html){
    ...
    html.on("click.superheroes","[data-action]",async e=>{
      ...
      try{
        ...
        // УНИВЕРСАЛЬНАЯ КНОПКА БИБЛИОТЕКИ (Способности или Особенности)
  ...
  async _addListRow(type){
    ...
    if (type === "powers") arr[arr.length - 1].cost = 0;
    await this.actor.update({["system."+type]:arr},{render:false});
    this.render(false);
  }
...
/* --- АВТОЗАПОЛНЕНИЕ БИБЛИОТЕК (Особенности и Способности) --- */
