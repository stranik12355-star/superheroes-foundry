...
async function renderSuperRoll(data){
  ...
  const html=await foundry.applications.handlebars.renderTemplate(
    "systems/superheroes/templates/roll-card.hbs",payload
  );
...
async function roll3d6(actor,label,statKey){
  ...
  const totalDice=critical ? (dice[0] + 6 + dice[2]) : dice.reduce((a,b)=>a+b,0);
...
async function rerollDie(messageId,dieIndex,mode="reroll"){
  ...
  flag.rerolled=true;
  flag.lastReroll={die:dieIndex,mode};
  const html=await foundry.applications.handlebars.renderTemplate(
    "systems/superheroes/templates/roll-card.hbs",flag
  );
...
class SuperheroesActorSheet extends ActorSheet {
  ...
  activateListeners(html){
    ...
    html.find("[data-action='send-item']").click(ev=>{
      this._sendListItem(ev.currentTarget.dataset.type,Number(ev.currentTarget.dataset.index));
    });
    ...
    html.find("input[data-action='resource-current']").change(async ev=>{
      ...
      if(field==="karma"){
        ...
        await this.actor.update({"system.karma":val});
        return;
      }
      ...
      buttons:{
        save:{label:"Сохранить",callback:async html=>{
          await this.actor.update({
            "system.health.bonus":Math.max(0,Number(html.find("#hb").val())||0),
            "system.focus.bonus":Math.max(0,Number(html.find("#fb").val())||0),
  ...
  async _editStat(key){
    ...
    const characteristicChange=statMod(s.value);
    const content=`<div class="sh-dialog">
      <label>Изменение характеристики (10 = 0)</label><input id="valueMod" type="number" value="${characteristicChange}">
    ...
    new Dialog({
      ...
      buttons:{
        save:{label:"Сохранить",callback:async html=>{
          ...
          const mod=Number(html.find("#valueMod").val())||0;
          await this.actor.update({
            [`${p}value`]:10+mod,
  ...
  async _sendListItem(type,index){
    const sys=mergeDefaults(this.actor.system);
    const item=sys[type]?.[index];
    if(!item?.name) return;
    const typeLabel=type==="powers"?"Способность":type==="traits"?"Черта":"Снаряжение";
    const meta=[];
    if(type==="powers"){
      if(item.movement) meta.push(`<span>Перемещение: ${foundry.utils.escapeHTML(item.movement)}</span>`);
      if(item.cost) meta.push(`<span>Стоимость: ${foundry.utils.escapeHTML(item.cost)}</span>`);
      if(item.range) meta.push(`<span>Дальность: ${foundry.utils.escapeHTML(item.range)}</span>`);
      if(item.damageType) meta.push(`<span>Тип урона: ${foundry.utils.escapeHTML(item.damageType)}</span>`);
    }
    const metaHtml=meta.length?`<div class="power-meta">${meta.join("")}</div>`:"";
    const content=`<div class="sh-chat-card">
      <div class="chat-title">${foundry.utils.escapeHTML(typeLabel)}</div>
      <h3 style="margin:0 0 8px">${foundry.utils.escapeHTML(item.name)}</h3>
      ${metaHtml}
      <div style="white-space:pre-wrap;color:#c8c8cc">${foundry.utils.escapeHTML(item.description||"")}</div>
    </div>`;
...
Hooks.once("init",()=>{
  Actors.registerSheet("superheroes",SuperheroesActorSheet,{
...
Hooks.on("renderChatMessage",(message,html)=>{
  ...
  root?.querySelectorAll("[data-action='reroll-die']").forEach(btn=>{
    btn.addEventListener("click",()=>rerollDie(btn.dataset.messageId,Number(btn.dataset.die),btn.dataset.mode),{once:true});
  });
});
