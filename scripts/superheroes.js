...
async function rerollSuperheroesDie(messageId,dieIndex,mode){
  ...
  // СОХРАНЯЕМ ДАННЫЕ О БРОСКЕ ДО ТОГО, КАК ФАУНДРИ ОБНОВИТ ЧАТ
  ...
  // 2. ОБНОВЛЕНИЕ ТРЕКЕРА. Используем сохраненные заранее данные.
  ...
  activateListeners(html) {
    ...
    html.on("click.superheroes","[data-action]",async e=>{
      ...
      try{
        ...
        if(a==="edit-portrait")return this._editPortrait(); // <== НОВЫЙ ОБРАБОТЧИК КАРТИНКИ
