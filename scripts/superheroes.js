 ...
 * Самостоятельная система. 3d6, центральная кость — критическая.
 */
const SH_DEFAULTS = {
  rank: 1,
  karma: 1,
  ...
  initiative: {bonus: 0},
  biography: {
    name: "", nickname: "", age: "", gender: "", height: "", weight: "",
    eyes: "", hair: "", history: "", notes: ""
  },
  stats: {
...
function mergeDefaults(source={}) {
  ...
  for (const s of Object.keys(d.stats)) {
    d.stats[s].value = Number.isFinite(Number(d.stats[s].value)) ? Number(d.stats[s].value) : 10;
    for (const k of ["defense","nonCombat","hit","multiplier","stable"])
      d.stats[s][k] = Number.isFinite(Number(d.stats[s][k])) ? Number(d.stats[s][k]) : 0;
  }
  d.rank = Math.min(5, Math.max(1, Number(d.rank)||1));
  d.karma = Number.isFinite(Number(d.karma)) ? Number(d.karma) : d.rank;
...
function fmtSigned(n){ return Number(n) >= 0 ? `+${Number(n)}` : `${Number(n)}`; }
...
async function roll3d6(actor, label, statKey){
  ...
  const totalDice = dice[1]===1 ? 6 : dice.reduce((a,b)=>a+b,0);
  const total = totalDice + s.mod;
  await renderSuperRoll({
    actor, kind:"check", label, statKey, dice,
    modifier:s.mod, total, critical:dice[1]===1
  });
}
...
async function rollAttack(actor, statKey){
  ...
  await renderSuperRoll({
    actor, kind:"attack", label:`Атака — ${game.i18n.localize(`SUPERHEROES.Stat.${statKey}`)}`,
    statKey, dice:[d6], multiplier:s.multiplier, modifier:s.stable, total, critical:false
  });
}

async function renderSuperRoll(data){
  const initial = foundry.utils.deepClone(data);
  const msg = await ChatMessage.create({
    speaker: ChatMessage.getSpeaker({actor:data.actor}),
    content: "<div class=\"sh-chat-card\"><div class=\"chat-title\">Бросок…</div></div>",
    flags: {"superheroes": {roll:initial}}
  });
  const payload = foundry.utils.deepClone(initial);
  payload._id = msg.id;
  const html = await foundry.applications.handlebars.renderTemplate("systems/superheroes/templates/roll-card.hbs", payload);
...
async function rerollDie(messageId, dieIndex){
  ...
  if (!flag || flag.kind!=="check") return;
  if (dieIndex < 0 || dieIndex > 2) return;
  flag.dice[dieIndex] = Math.floor(Math.random()*6)+1;
  flag.critical = flag.dice[1]===1;
  const sum = flag.dice[1]===1 ? 6 : flag.dice.reduce((a,b)=>a+b,0);
  flag.total = sum + Number(flag.modifier||0);
  flag.rerolled = true;
  const html = await foundry.applications.handlebars.renderTemplate("systems/superheroes/templates/roll-card.hbs", flag);
...
class SuperheroesActorSheet extends ActorSheet {
  ...
  getData(options={}){
    ...
    const maxKarma = system.rank;
    system.health.value = Math.min(Number(system.health.value)||0, maxHP);
    system.focus.value = Math.min(Number(system.focus.value)||0, maxFocus);
    const stats = {};
    for (const [key,s] of Object.entries(system.stats)) stats[key] = {...s, ...statDerived(this.actor,key)};
    const initiative = system.stats.vigilance.value - 10 + Number(system.initiative.bonus||0);
    data.system = system;
    data.derived = {maxHP,maxFocus,maxKarma,initiative,stats};
    data.statList = Object.keys(stats).map(key=>({key,label:game.i18n.localize(`SUPERHEROES.Stat.${key}`)}));
  ...
  activateListeners(html){
    ...
    html.find("[data-action='roll-check']").click(ev=>{
      const key=ev.currentTarget.dataset.stat; roll3d6(this.actor, game.i18n.localize(`SUPERHEROES.Stat.${key}`), key);
    });
    html.find("[data-action='roll-noncombat']").click(ev=>{
      const key=ev.currentTarget.dataset.stat;
      this._realNonCombatRoll(key);
    });
    ...
    html.find("input[data-action='resource-current']").change(async ev=>{
      ...
      let val=Math.max(0,Number(ev.currentTarget.value)||0);
      const sys=mergeDefaults(this.actor.system);
      const max=field==="health" ? resourceMax(sys.stats.endurance.value,sys.health.bonus) : resourceMax(sys.stats.vigilance.value,sys.focus.bonus);
  ...
  async _realNonCombatRoll(key){
    const s=statDerived(this.actor,key);
    const dice=[1,1,1].map(()=>Math.floor(Math.random()*6)+1);
    const totalDice=dice[1]===1?6:dice.reduce((a,b)=>a+b,0);
    await renderSuperRoll({actor:this.actor,kind:"check",label:`${game.i18n.localize(`SUPERHEROES.Stat.${key}`)} — Вне боя`,statKey:key,dice,modifier:s.mod,total:totalDice+s.mod,critical:dice[1]===1,nonCombat:true});
  }

  async _editRank(){
    const sys=mergeDefaults(this.actor.system);
    const content=`<div class="sh-dialog"><label>Ранг (1–5)</label><input id="rank" type="number" min="1" max="5" value="${sys.rank}"></div>`;
    new Dialog({title:"Ранг",content,buttons:{save:{label:"Сохранить",callback:async html=>{const r=Math.min(5,Math.max(1,Number(html.find("#rank").val())||1)); await this.actor.update({"system.rank":r});}},cancel:{label:"Отмена"}}}).render(true);
  }
  ...
  async _editResources(){
    ...
    new Dialog({title:"Настройки ресурсов",content,buttons:{save:{label:"Сохранить",callback:async html=>{
      await this.actor.update({"system.health.bonus":Number(html.find("#hb").val())||0,"system.focus.bonus":Number(html.find("#fb").val())||0,"system.initiative.bonus":Number(html.find("#ib").val())||0});
    }},cancel:{label:"Отмена"}}}).render(true);
  }
  ...
  async _editStat(key){
    ...
    new Dialog({title:`Настройки — ${game.i18n.localize(`SUPERHEROES.Stat.${key}`)}`,content,buttons:{save:{label:"Сохранить",callback:async html=>{
  ...
  async _editBiography(){
    ...
    new Dialog({title:"Биография",content,buttons:{save:{label:"Сохранить",callback:async html=>{
      const update={}; for(const [k] of fields) update[`system.biography.${k}`]=html.find(`#${k}`).val();
      update["system.biography.history"]=html.find("#history").val(); update["system.biography.notes"]=html.find("#notes").val();
      await this.actor.update(update);
    }},cancel:{label:"Отмена"}}}).render(true);
  }

  async _editListItem(type,index){
    const sys=mergeDefaults(this.actor.system), item=index>=0 ? sys[type][index] : {name:"",description:""};
    ...
    new Dialog({title:`${index<0?"Добавить":"Изменить"} — ${isPower?"Способность":type==="traits"?"Черта":"Снаряжение"}`,content,buttons:{save:{label:"Сохранить",callback:async html=>{
      const next={name:html.find("#name").val(),description:html.find("#description").val()};
      if(isPower){next.movement=html.find("#movement").val();next.cost=html.find("#cost").val();next.range=html.find("#range").val();next.damageType=html.find("#damageType").val();}
      const arr=deepClone(sys[type]); if(index<0) arr.push(next); else arr[index]=next;
      await this.actor.update({[`system.${type}`]:arr});
    }},cancel:{label:"Отмена"}}}).render(true);
  }

  async _deleteListItem(type,index){
    const sys=mergeDefaults(this.actor.system), arr=deepClone(sys[type]); arr.splice(index,1);
    await this.actor.update({[`system.${type}`]:arr});
  }
  ...
  async _editSystem(){
    ...
    const content=`<div class="sh-dialog"><p>Здесь можно изменить текущие ресурсы и служебные значения.</p>
      <label>Текущая Карма</label><input id="karma" type="number" value="${sys.karma}">
    ...
    new Dialog({title:"Общие настройки",content,buttons:{save:{label:"Сохранить",callback:async html=>{
      await this.actor.update({"system.karma":Number(html.find("#karma").val())||0,"system.health.bonus":Number(html.find("#hb").val())||0,"system.focus.bonus":Number(html.find("#fb").val())||0,"system.initiative.bonus":Number(html.find("#ib").val())||0});
    }},cancel:{label:"Отмена"}}}).render(true);
  }
}
...
Hooks.on("preCreateActor",(actor,data,options,userId)=>{
  ...
  const current=mergeDefaults(actor.system);
  actor.updateSource({system:current});
});
...
Hooks.on("renderActorSheet",(app,html)=>{
  ...
  root.querySelectorAll("[data-action='reroll-die']").forEach(btn=>{
    btn.onclick=()=>rerollDie(btn.dataset.messageId,Number(btn.dataset.die));
  });
});
