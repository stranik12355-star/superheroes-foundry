...
async function make3d6Roll(actor, formula, flavor, flags={}){
  ...
  if (flags.kind === "initiative") {
    ...
    const tokenDoc = actor.isToken ? actor.token : actor.getActiveTokens()[0]?.document;
    let combatant = combat.combatants.find(c => (tokenDoc && c.tokenId === tokenDoc.id) || c.actorId === actor.id);
    
    if (!combatant) {
      const combatantData = { actorId: actor.id, initiative: roll.total };
      if (tokenDoc) {
...
async function rerollSuperheroesDie(messageId,dieIndex,mode){
  ...
  // ПЕРЕБРОС ИНИЦИАТИВЫ: Синхронизация с трекером (через нативный getFlag)
  const kind = chatMessage.getFlag("superheroes", "kind");
  if (kind === "initiative" && game.combat) {
    const speaker = chatMessage.speaker;
    const tokenDocId = speaker.token;
    const actorId = speaker.actor;
    
    // Ищем участника боя по токену или актеру
    const combatant = game.combat.combatants.find(c => (tokenDocId && c.tokenId === tokenDocId) || c.actorId === actorId);
    
    if (combatant) {
      // Обновляем трекер на лету
...
// ==========================================
// ИНСТРУМЕНТ: БЫСТРОЕ ДОБАВЛЕНИЕ В БОЙ
// ==========================================
