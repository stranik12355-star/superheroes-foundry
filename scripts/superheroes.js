...
async function make3d6Roll(actor, formula, flavor, flags={}){
  ...
  if (flags.kind === "initiative") {
    // 1. Убеждаемся, что трекер существует
...
// === ПЕРЕБРОС: Хирургическое обновление инициативы ===
async function rerollSuperheroesDie(messageId,dieIndex,mode){
  ...
  // 1. ЖЕЛЕЗОБЕТОННАЯ МАТЕМАТИКА. Вычисляем и сохраняем точную цифру.
  if(newRollResult.active) {
    roll._total = roll.total - oldResult + newResult;
  }
  const exactFinalTotal = roll._total === 1 ? 6 : roll._total;
  ...
  // 2. ОБНОВЛЕНИЕ ТРЕКЕРА. Читаем флаг только из свежего merged-объекта!
  const flags = merged.flags?.superheroes || {};
  if (flags.kind === "initiative" && game.combat) {
    const actorId = merged.speaker?.actor;
    const tokenId = merged.speaker?.token;
...
// ==========================================
// ИНСТРУМЕНТ: БЫСТРОЕ ДОБАВЛЕНИЕ В БОЙ (БЕЗ ОШИБОК)
// ==========================================
