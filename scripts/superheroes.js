...
async function make3d6Roll(actor,formula,flavor,flags={}){
  ...
  // Если это инициатива, и персонаж в бою, обновляем трекер сразу
...
Hooks.once("ready", () => {
  // Регистрируем временную настройку для переключателя
...
Hooks.on("controlToken", async (token, controlled) => {
  if (controlled && game.settings.get("core", "fastCombatActive") && game.combat) {
    // Если токен уже в бою - убираем. Если нет - добавляем.
    if (token.inCombat) {
      const combatant = game.combat.combatants.find(c => c.tokenId === token.id);
      if (combatant) await combatant.delete();
      ui.notifications.info(`${token.name} убран из боя.`);
    } else {
      await token.toggleCombat();
      ui.notifications.info(`${token.name} добавлен в бой.`);
    }
  }
});
