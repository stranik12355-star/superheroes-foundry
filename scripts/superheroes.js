...
// ===== ОБНОВЛЕНО: СЕКРЕТНОЕ И КРАСИВОЕ РАЗРЕШЕНИЕ АТАКИ =====
async function resolveAttack(messageId, htmlRoot) {
  ...
  for (const target of targets) {
      ...
      if (attackTotal >= defenseValue) {
          ...
          // Красивая визуальная карточка с кубиком
          const hitHtml = `
          <div class="superheroes dice-roll">
            <div style="text-align: center; color: #a4d65e; font-weight: bold; font-size: 13px; margin-bottom: 5px; padding: 4px; background: rgba(164, 214, 94, 0.1); border-radius: 4px; border: 1px solid rgba(164, 214, 94, 0.3);">
              ✅ ПРОБИЛ: ${attackerName} ➔ ${target.name}
            </div>
          ...
          await dmgRoll.toMessage({ speaker: message.speaker, flavor: hitHtml });
          
      } else {
          // ПРОМАХ! Строгая стильная карточка без лишней инфы
          const missHtml = `
          <div style="background: #111; padding: 8px; border-left: 3px solid #d34a53; border-radius: 4px; color: #eee; font-family: sans-serif; text-align: center;">
              <div style="color: #d34a53; font-weight: bold; font-size: 13px;">❌ НЕ ПРОБИЛ</div>
              <div style="font-size: 12px; color: #aaa; margin-top: 3px;"><b>${attackerName}</b> ➔ <b>${target.name}</b></div>
          </div>`;
