...
function mergeDefaults(source={}) {
  ...
  // Добавляем ID всем старым предметам, если их нет
...
class SuperheroesActorSheet extends ActorSheet {
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      dragDrop: [{dragSelector: ".list-view-item", dropSelector: ".folder-drop-zone, .list-view-item"}]
    });
  }
  ...
  activateListeners(html) {
    ...
    // --- DRAG & DROP ПЕРЕРАБОТАННЫЙ ---
    const listItems = html.find('.list-view-item');
    const dropZones = html.find('.folder-drop-zone');
    ...
    listItems.on('dragstart', (e) => {
      ...
      const dragData = {
        ...
        id: el.dataset.id,
        sourceType: el.classList.contains('gear-folder-head') ? 'folder' : 'item'
      };
    ...
    html.find('.folder-drop-zone, .list-view-item').on('drop', async (e) => {
      ...
      if (data.type === "SortItem") {
        ...
        if (dropTarget.classList.contains('folder-drop-zone')) {
          ...
          arr.push(item); // Добавляем в конец папки/корня
        } 
        // Если бросили на другой предмет
          ...
          if (listType === "gear") item.folderId = targetItem.folderId; // Берем папку цели
    ...
    html.on("change.superheroes","[data-edit-field]",async e=>{
      ...
      // Обработка вложенных свойств (например hp.max)
      if (field.includes('.')) {
