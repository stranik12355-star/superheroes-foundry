...
// ===== РАЗРЕШЕНИЕ АТАКИ С УЧЕТОМ ПРАВ ДОСТУПА ЧЕРЕЗ СОКЕТ =====
async function resolveAttack(messageId, htmlRoot) {
  ...
  for (const target of targets) {
      ...
      if (attackTotal >= defenseValue) {
          ...
          // Проверка прав: если ГМ или владелец — обновляем напрямую, иначе слаем запрос ГМу по сокету
          if (game.user.isGM || targetActor.isOwner) {
              await targetActor.update({[`system.${resourceKey}.value`]: newHp});
          } else {
              game.socket.emit("system.superheroes", {
                  action: "applyDamage",
...
Hooks.once("ready", function() {
  ...
  // Слушатель сокетов для получения урона от игроков на стороне ГМа
  game.socket.on("system.superheroes", async data => {
    if (data.action === "applyDamage" && game.user.isGM) {
      const targetActor = game.actors.get(data.actorId);
      if (targetActor) {
        await targetActor.update({ [`system.${data.resourceKey}.value`]: data.newHp });
      }
    }
  });
});
...
Hooks.on("preCreateActor", function(actor) {
  ...
  actor.updateSource({
    ...
    prototypeToken: {
      actorLink: true,                            
      displayName: 20, 
      displayBars: 20, // По умолчанию: При наведении владельцем
