...
// ===== НОВОЕ: Автоматическое разрешение атаки (Логика) =====
async function resolveAttack(messageId, htmlRoot) {
  ...
  // Читаем выбор из выпадающих списков
  ...
  for (const target of targets) {
      ...
      // Считаем актуальную защиту цели с учетом возможных модификаторов (Берсерк и т.д.)
      ...
      let reportHtml = `<div style="background: #111; padding: 8px; border-left: 3px solid #888; border-radius: 4px; color: #eee; font-family: sans-serif;">
          <h4 style="margin: 0 0 5px 0; border-bottom: 1px solid #333; color: #aaa; font-size: 12px; text-transform: uppercase;">ОТЧЁТ ОБ АТАКЕ</h4>
          <div style="font-size: 13px; margin-bottom: 5px;"><b>${attackerName}</b> атакует <b>${target.name}</b>!</div>
          <div style="font-size: 12px; margin-bottom: 5px;">🎯 <b>Проверка:</b> Атака <b>${attackTotal}</b> против Защиты (${defNameNames[defenseKey]}) <b>${defenseValue}</b></div>
      `;
      ...
      if (attackTotal >= defenseValue) {
          ...
          // Автоматически вычитаем здоровье/фокус у цели
          const newHp = Math.max(0, sys[resourceKey].value - finalDamage);
          await targetActor.update({[`system.${resourceKey}.value`]: newHp});
          
          reportHtml += `
              <div style="color: #a4d65e; font-weight: bold; margin-bottom: 5px;">🟢 ПРОБИЛ ЗАЩИТУ!</div>
              <div style="font-size: 11px; color: #999; margin-bottom: 5px;">🎲 1d6 = ${d6Result} | Множ. Атакующего = ${mult} | Снижение цели = ${reduction}</div>
              <div style="font-size: 14px; background: #8d2b33; padding: 4px; text-align: center; border-radius: 3px;">🩸 Нанесено <b>${finalDamage}</b> урона по <b>${resNameNames[resourceKey]}</b></div>
          </div>`;
          
          // Выводим бросок урона и отчёт в чат
          await dmgRoll.toMessage({ speaker: message.speaker, flavor: reportHtml });
          
      } else {
          // ПРОМАХ!
          reportHtml += `
              <div style="color: #d34a53; font-weight: bold;">🛑 АТАКА НЕ ПРОБИЛА ЗАЩИТУ!</div>
              <div style="font-size: 12px; color: #888;">Урона нет.</div>
          </div>`;
          
          await ChatMessage.create({ speaker: message.speaker, content: reportHtml });
      }
  }
}
// =================================================================
...
class SuperheroesActorSheet extends ActorSheet {
  ...
  activateListeners(html) {
    ...
    // Перетаскивание макроса броска на панель
...
// ===== ОБНОВЛЕНО: Добавление выпадающих списков прямо в чат карточку =====
