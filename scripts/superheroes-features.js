...
Hooks.on("hoverToken", (token, hovered) => {
    ...
    const updatePosition = () => {
        ...
        // Надёжный расчёт позиции через координаты токена
        const x = token.center.x;
        const y = token.document.y - 10;
...
// 6. Проигрывание звука поднятой руки для всех пользователей (ИСПРАВЛЕНО на game.audio)
